...
#
# For Q7, we first gave out the list since we need to generate instances later
...
def count_tree_node(subroot):
    if subroot is None:
        raise Exception("no leaf?")
    if subroot.isleaf:
        return 1
    else:
        left_count = count_tree_node(subroot.left_node)
        right_count = count_tree_node(subroot.right_node)
        return 1 + left_count + right_count



def test_set_error(tree_root, test_set):
    correct_count = 0
    ...
    return 1 - (correct_count / len(test_set))
...
if __name__ == "__main__":
    ...
    # process it
    shuffled = list(random.permutation(inst_list))
    test_set = shuffled[8192:]
    print(len(test_set))
    train_set_32 = shuffled[:32]
    train_set_128 = shuffled[:128]
    train_set_512 = shuffled[:512]
    train_set_2048= shuffled[:2048]
    train_set_8192 = shuffled[:8192]
    training_sets = [train_set_32, train_set_128, train_set_512, train_set_2048, train_set_8192]
    #train it
    tree_root_32 = GenerateSubTree([train_set_32, copy.deepcopy(train_set_32)], 0)
    tree_root_128 = GenerateSubTree([train_set_128, copy.deepcopy(train_set_128)], 0)
    tree_root_512 = GenerateSubTree([train_set_512, copy.deepcopy(train_set_512)], 0)
    tree_root_2048 = GenerateSubTree([train_set_2048, copy.deepcopy(train_set_2048)], 0)
    tree_root_8192 = GenerateSubTree([train_set_8192, copy.deepcopy(train_set_8192)], 0)
    resulting_trees = [tree_root_32, tree_root_128, tree_root_512, tree_root_2048, tree_root_8192]
    # examine them
    print("set name   :   n,   node_count,   err_n")
    curve_plot_x = []
    curve_plot_y = []
    for i in range(0, 5):
        train_set_size = int(32 * math.pow(4,i))
        node_count = count_tree_node(resulting_trees[i])
        err_n = test_set_error(resulting_trees[i], test_set)
        print("D"+str(train_set_size)+" : "+str(train_set_size)+",    "+str(node_count)+",  "+str(err_n))
        make_plot(training_sets[i], resulting_trees[i], "Q7_D"+str(train_set_size))
        curve_plot_x.append(train_set_size)
        curve_plot_y.append(err_n)
    fig, axes = plt.subplots()
    fig.set_figheight(9)
